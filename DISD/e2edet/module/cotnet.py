...
from .CoTNetBlock import CotLayer ####修改了
...
model_urls = {
    'cotnet50': '/media/dell/5969A0774543D3F6/ZT/BoxeR-main/e2edet/module/cotnet50.pth',
}
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        first_planes = width // reduce_first  #xinjia
        ...
        if stride > 1:
            ...
            self.avd = None
        self.cot_layer = CotLayer(dim=width, kernel_size=3)
        ...
        # self.cot_layer = CotLayer(dim=planes, kernel_size=3)
...
def _load_state_dict(arch, pretrained, pretrained_path, model_data_dir):
    ...
    if pretrained:
        ...
        if is_master():
            ...
            torch.load(model_urls[arch])    #gai,xinjia
        synchronize()
        # state_dict = load_state_dict_from_url(model_urls[arch])
        state_dict = torch.load(model_urls[arch])   #gai,新加
...
def _build_model(layers, num_channels, state_dict=None, **kwargs):
    ...
    if state_dict is not None:
        ...
        print("Loaded pretrained cotnet:", named_tuple)

    return model


def build_cotnet(config):
    arch = config["type"]
        ...
        if arch == "cotnet50":
            ...
            if pretrained:
                raise RuntimeError(
                    ...
                    "rcotnet50|cotnet101. Found {}".format(arch)
                )
